...
class Setup{
		...
		if($dbname):
			...
			$this->CI->db->query("
				...
				(
					...
					`inovice_customer_kode`,`invoice_tanggal_faktur`,`invoice_keterangan`,`invoice_tanggal_pengantaran`,
			...
			$this->CI->db->query("
				...
				(
					`invoice_cabang_id`, `invoice_type`,`invoice_no_order`,`invoice_no_so`,`invoice_sales_id`,
					`inovice_customer_kode`,`invoice_tanggal_faktur`,`invoice_keterangan`,`invoice_tanggal_input`,
					`invoice_hari_jatuh_tempo`,`invoice_kredit`,`invoice_biaya_lain`,`invoice_total_setelah_pajak`,
					`invoice_uang_muka`,`invoice_piutang`,`invoice_kode_akun_lunas`,
					`invoice_status_pembayaran`,
	...
	public function dump_stock_product($dbname)
	{
		...
		if($dbname):
			...
			$this->CI->db->query("
				...
				FROM ".$this->CI->db->dbprefix."data_stock_product WHERE stock_type_transaksi = 'beli'
			...
			foreach($query as $result):
				...
				$this->CI->db->query("
					UPDATE ".$this->CI->db->dbprefix."data_stock SET 
						`stok_jumlah` = '".$result['jumlah_stok_akhir']."'
					WHERE stok_id = '".$result['stok_id']."'
				");
			...
			$query	=	$this->CI->db->query("
							...
							SELECT *
								,(SELECT product_nama FROM ".$this->CI->db->dbprefix."setting_produk WHERE product_kode = a.stock_produk_kode) as nama_produk 
		...
		if($dbname):
			...
			$this->CI->db->query("
				...
				(
					...
					`penawaran_status`,`penawaran_user_id`
				)
			...
			$this->CI->db->query("
				...
				(
					`penawaran_detail_cabang_id`,`penawaran_nomer`,`penawaran_product_kode`,`penawaran_jumlah`,`penawaran_status`,
					`penawaran_product_nama`
				)
	...
	public function dump_bank($dbname)
	{
		if($dbname):
			...
			$this->CI->db->query("
				DELETE FROM ".$this->CI->db->dbprefix."setting_bank
				WHERE bank_cabang_id = '".$cabang_id."'
			");

			$this->CI->db->query("
				
				INSERT INTO ".$this->CI->db->dbprefix."setting_bank 
				(
		...
		if($dbname):
			...
			$this->CI->db->query("
				...
				(
					...
					`category_rekening_persediaan`,`category_gambar`,`category_user_id`,`category_dealer`
				)
	...
	public function dump_supplier($dbname)
	{
		if($dbname):
			...
			$this->CI->db->query("
				TRUNCATE ".$this->CI->db->dbprefix."setting_supplier 
			");

			//ambil data dari cabang sesuai database name post
			$this->CI->db->query("
				
				INSERT INTO ".$this->CI->db->dbprefix."setting_supplier 
				(
	...
	public function dump_customer($dbname)
	{
		if($dbname):
			...
			$this->CI->db->query("
				DELETE FROM ".$this->CI->db->dbprefix."data_customer 
				WHERE customer_cabang_id = '".$cabang_id."'
			");

			//ambil data dari cabang sesuai database name post
			$this->CI->db->query("
				
				INSERT INTO ".$this->CI->db->dbprefix."data_customer 
				(
					`customer_cabang_id`,`customer_code`,`customer_type`,`customer_nama`,`customer_alamat`,`customer_telepon`,
					`customer_nama_kota`,`customer_email`,`customer_notes`,
					`customer_instansi_nama`,`customer_instansi_alamat`,`customer_instansi_nama_kota`,`customer_instansi_telepon`,
					`customer_instansi_email`,`customer_instansi_web`,
					`customer_old_id`
				)
	...
	public function generate_user()
	{
		...
		foreach($query->result_array() as $result):
			...
			$this->CI->db->insert($this->CI->db->dbprefix."user", $data);
